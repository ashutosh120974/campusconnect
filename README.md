...
> ⚠️ **Project status: Phase 1 foundation.** This repository contains a working, production-style foundation (monorepo, auth, RBAC, core data models, REST API, real-time chat scaffold, and a premium responsive UI). It is **not** the full feature set from the original brief yet — see [Roadmap](#roadmap). The architecture is deliberately modular so the remaining phases can be layered in.
...
## Security
JWT access/refresh tokens, bcrypt password hashing, Helmet, CORS allow-list, global + auth rate limiting, Mongo injection sanitization, HTTP param pollution protection, Zod request validation, httpOnly refresh cookie.

## Roadmap

Phase 1 (this repo) delivers the foundation. Remaining phases from the product brief:

- **Verification pipeline** — college-email OTP, student-ID upload to S3, OCR extraction + matching, admin review queue, fraud detection.
